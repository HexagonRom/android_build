...
""" % bcb_dev)
  ...
  script.Print("    _   _                                             ");
  script.Print("   | | | |                                            ")
  script.Print("   | |_| |  ___ __  __ __ _   __ _   ___    _ __      ")
  script.Print("   |  _  | / _ \\ \/ // _\`| / _  \/  _  \ | '_ \     ")
  script.Print("   | | | ||  __/ >  <| (_| || (_| || (_) | | | | |    ")
  script.Print("   \_| |_/ \___|/_/\_\\__,_| \__, | \___/  |_| |_|    ")
  script.Print("                              __/ |                   ")
  script.Print("                             |___/                    ");

  hexagonrom = GetBuildProp("ro.hexagon.version", OPTIONS.info_dict)
  if os.getenv("HEXAGON_BUILDTYPE") is not None:
    build = ' '.join(hexagonrom.split('_')[3].split('-')).title()
    script.Print("*   Version: %s"%(build));
  elif os.getenv("HEXAGON_NIGHTLY") is not None:
    build = ' '.join(hexagonrom.split('_')[2:]).title()
    script.Print("*   Version: %s"%(build));
  else:
    build = GetBuildProp("ro.build.date", OPTIONS.info_dict)
    script.Print("******************************************************");
    script.Print("****************    HEXAGON   BUILD   ****************");
    script.Print("******************************************************");
  ...
  if block_based:
    script.Print("Flashing Dolby...")
    common.ZipWriteStr(output_zip, "dolby/dolby.zip",
                   ""+input_zip.read("SYSTEM/addon.d/dolby.zip"))
    script.Mount("/system")
    script.DolbyScript()
...
""" % bcb_dev)
  ...
  common.ZipWriteStr(output_zip, "META-INF/org/hexagon/releasekey",
