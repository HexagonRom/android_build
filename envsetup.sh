...
- cmremote: Add git remote for HEXAGON Gerrit Review
- cmgerrit: A Git wrapper that fetches/pushes patch from/to HEXAGON Gerrit Review
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the HEXAGON model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var HEXAGON_VERSION)
        ZIPFILE=hexagon-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
