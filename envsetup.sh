...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/hexagon/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/hexagon/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/hexagon/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${HEXAGON_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the HEXAGON github
        T=$(gettop)
        pushd $T > /dev/null
        vendor/hexagon/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        vendor/hexagon/build/tools/roomservice.py $product true
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/hexagon/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
